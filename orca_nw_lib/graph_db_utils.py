...
def create_lldp_relations(topology):
    for device, neighbors in topology.items():
        for nbr in neighbors:
            local_intfc = getInterfaceOfDeviceFromDB(device.mgt_ip, nbr.get("local_port"))

            nbr_device = nbr.get("nbr_device")
            nbr_intfc = getInterfaceOfDeviceFromDB(nbr_device.mgt_ip, nbr.get("nbr_port"))
...
def create_mclag_peerlink_relations():
    for local_dev in getAllDevicesFromDB() or []:
        ...
        if mclag_local:
            ...
            port_chnl_local=getPortChnlOfDeviceFromDB(local_dev.mgt_ip,peer_link_local)
            ...
            port_chnl_remote=getPortChnlOfDeviceFromDB(local_dev.mgt_ip,peer_link_remote)
...
def insert_device_port_groups_in_db(device: Device = None, port_groups: dict = None):
    for pg, mem_intfcs in port_groups.items():
        ...
        for if_name in mem_intfcs:
            intf = getInterfaceOfDeviceFromDB(device.mgt_ip, if_name)
            if intf:
...
def set_interface_config_in_db(device_ip:str,if_name:str,enable:bool=None,mtu=None,speed:Speed=None):
...
def getMCLAGOfDeviceFromDB(device_ip: str, domain_id: str) -> PortChannel:
    device = getDeviceFromDB(device_ip)
    return (
        getDeviceFromDB(device_ip).mclags.get_or_none(domain_id=domain_id) if device else None
    )
